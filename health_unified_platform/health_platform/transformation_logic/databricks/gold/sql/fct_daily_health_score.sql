...
--
-- Target: health_dw.gold.fct_daily_health_score (Databricks)
-- Materialization: CREATE OR REPLACE VIEW (always fresh from silver)
-- Grain: one row per day
-- Weights: sleep 0.35, readiness 0.35, activity 0.30
-- =============================================================================
...
FROM health_dw.silver.daily_sleep s
FULL OUTER JOIN health_dw.silver.daily_readiness r
    ON s.sk_date = r.sk_date
FULL OUTER JOIN health_dw.silver.daily_activity a
    ON COALESCE(s.sk_date, r.sk_date) = a.sk_date
LEFT JOIN health_dw.silver.daily_stress st
