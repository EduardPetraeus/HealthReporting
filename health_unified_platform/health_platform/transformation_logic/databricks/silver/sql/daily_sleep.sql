-- =============================================================================
-- daily_sleep.sql
-- Silver: Oura daily sleep score and contributors
--
-- Source: workspace.default.oura_daily_sleep
-- TODO: Update source to health_dw.bronze.stg_oura_daily_sleep when bronze is ready
--
-- Business key: (id, day)
-- Change detection: row_hash over sleep_score, timestamp, and all sleep contributors
-- =============================================================================

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS health_dw.silver.daily_sleep (
...
WHEN NOT MATCHED THEN
    INSERT (
        sk_date, id, day, sleep_score, timestamp, deep_sleep, efficiency,
        latency, rem_sleep, restfulness, timing, total_sleep,
        row_hash, load_datetime, update_datetime
    )
    VALUES (
        source.sk_date, source.id, source.day, source.sleep_score, source.timestamp,
        source.deep_sleep, source.efficiency, source.latency, source.rem_sleep,
        source.restfulness, source.timing, source.total_sleep,
