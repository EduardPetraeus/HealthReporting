-- =============================================================================
-- hrv.sql
-- Silver: Apple Health heart rate variability (SDNN)
--
-- Source: health_dw.bronze.stg_apple_health_hrv
--
-- Business key: business_key_hash (timestamp + source_name)
-- Change detection: row_hash over hrv_ms
-- =============================================================================

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS health_dw.silver.hrv (
...
CREATE OR REPLACE TABLE health_dw.silver.hrv_staging AS
WITH deduped AS (
    SELECT
        *,
        ROW_NUMBER() OVER (
    ...
    FROM health_dw.bronze.stg_apple_health_hrv
...
SELECT
    ...
    CAST(value AS DOUBLE)                                  AS hrv_ms,
...
MERGE INTO health_dw.silver.hrv AS target
USING health_dw.silver.hrv_staging AS source
...
WHEN NOT MATCHED THEN
    ...
    VALUES (
        ...
        source.hrv_ms, source.source_name,
...
DROP TABLE IF EXISTS health_dw.silver.hrv_staging;
