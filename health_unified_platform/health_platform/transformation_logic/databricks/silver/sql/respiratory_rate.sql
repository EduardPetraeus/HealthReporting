...
-- Silver: Apple Health respiratory rate measurements
--
-- Source: health_dw.bronze.stg_apple_health_respiratory_rate
--
-- Business key: business_key_hash (timestamp + source_name)
-- Change detection: row_hash over breaths_per_min
-- =============================================================================

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS health_dw.silver.respiratory_rate (
    sk_date           INTEGER   NOT NULL,
    sk_time           STRING    NOT NULL,
    timestamp         TIMESTAMP,
    end_timestamp     TIMESTAMP,
    breaths_per_min   DOUBLE,
    source_name       STRING,
...
CREATE OR REPLACE TABLE health_dw.silver.respiratory_rate_staging AS
WITH deduped AS (
    SELECT
        *,
        ROW_NUMBER() OVER (
    ...
    FROM health_dw.bronze.stg_apple_health_respiratory_rate
...
SELECT
    ...
    CAST(value AS DOUBLE)                                  AS breaths_per_min,
...
MERGE INTO health_dw.silver.respiratory_rate AS target
USING health_dw.silver.respiratory_rate_staging AS source
...
WHEN MATCHED AND target.row_hash <> source.row_hash THEN
    UPDATE SET
        ...
        target.breaths_per_min   = source.breaths_per_min,
...
WHEN NOT MATCHED THEN
    INSERT (
        sk_date, sk_time, timestamp, end_timestamp, breaths_per_min, source_name,
        business_key_hash, row_hash, load_datetime, update_datetime
    )
    VALUES (
        source.sk_date, source.sk_time, source.timestamp, source.end_timestamp,
        source.breaths_per_min, source.source_name,
...
DROP TABLE IF EXISTS health_dw.silver.respiratory_rate_staging;
