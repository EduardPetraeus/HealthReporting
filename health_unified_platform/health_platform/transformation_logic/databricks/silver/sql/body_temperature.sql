...
-- Silver: Apple Health body temperature readings
--
-- Source: health_dw.bronze.stg_apple_health_body_temperature
--
-- Business key: business_key_hash (timestamp + source_name)
-- Change detection: row_hash over temperature_degc
-- =============================================================================

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS health_dw.silver.body_temperature (
...
CREATE OR REPLACE TABLE health_dw.silver.body_temperature_staging AS
WITH deduped AS (
    SELECT
        *,
        ROW_NUMBER() OVER (
    ...
    FROM health_dw.bronze.stg_apple_health_body_temperature
...
SELECT
    ...
    CAST(value AS DOUBLE)                                  AS temperature_degc,
...
MERGE INTO health_dw.silver.body_temperature AS target
USING health_dw.silver.body_temperature_staging AS source
...
WHEN MATCHED AND target.row_hash <> source.row_hash THEN
    UPDATE SET
        ...
        target.temperature_degc  = source.temperature_degc,
...
WHEN NOT MATCHED THEN
    INSERT (
        sk_date, sk_time, timestamp, temperature_degc, source_name,
        business_key_hash, row_hash, load_datetime, update_datetime
    )
    VALUES (
        source.sk_date, source.sk_time, source.timestamp,
        source.temperature_degc, source.source_name,
...
DROP TABLE IF EXISTS health_dw.silver.body_temperature_staging;
