-- =============================================================================
-- daily_readiness.sql
-- Silver: Oura daily readiness score and contributors
--
-- Source: workspace.default.oura_daily_readiness
-- TODO: Update source to health_dw.bronze.stg_oura_daily_readiness when bronze is ready
--
-- Business key: (id, day)
-- Change detection: row_hash over score, temperature fields, and all contributors
-- =============================================================================

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS health_dw.silver.daily_readiness (
...
WHEN NOT MATCHED THEN
    INSERT (
        sk_date, id, day, readiness_score, temperature_deviation, temperature_trend_deviation,
        timestamp, activity_balance, body_temperature, hrv_balance, previous_day_activity,
        previous_night, recovery_index, resting_heart_rate, sleep_balance, sleep_regularity,
        row_hash, load_datetime, update_datetime
    )
    VALUES (
        source.sk_date, source.id, source.day, source.readiness_score,
        source.temperature_deviation, source.temperature_trend_deviation, source.timestamp,
        source.activity_balance, source.body_temperature, source.hrv_balance,
        source.previous_day_activity, source.previous_night, source.recovery_index,
        source.resting_heart_rate, source.sleep_balance, source.sleep_regularity,
