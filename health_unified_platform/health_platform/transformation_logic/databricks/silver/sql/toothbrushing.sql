-- =============================================================================
-- toothbrushing.sql
-- Silver: Apple Health toothbrushing events
--
-- Source: health_dw.bronze.stg_apple_health_toothbrushing
--
-- Business key: business_key_hash (timestamp + source_name)
-- Change detection: row_hash over duration_seconds
-- =============================================================================

-- COMMAND ----------

CREATE TABLE IF NOT EXISTS health_dw.silver.toothbrushing (
...
CREATE OR REPLACE TABLE health_dw.silver.toothbrushing_staging AS
WITH deduped AS (
    SELECT
        *,
        ROW_NUMBER() OVER (
    ...
    FROM health_dw.bronze.stg_apple_health_toothbrushing
...
SELECT
    ...
    sha2(
        coalesce(cast(duration_seconds AS STRING), ''), 256
    )                                                      AS row_hash,
...
MERGE INTO health_dw.silver.toothbrushing AS target
USING health_dw.silver.toothbrushing_staging AS source
...
WHEN NOT MATCHED THEN
    ...
    VALUES (
        ...
        source.duration_seconds, source.source_name,
...
DROP TABLE IF EXISTS health_dw.silver.toothbrushing_staging;
