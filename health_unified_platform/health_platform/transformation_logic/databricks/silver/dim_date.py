...
df = (
    df.withColumn(
        "sk_date", year("date") * 10000 + month("date") * 100 + dayofmonth("date")
    )
    ...
    .withColumn(
        ...
        when(
            (year("date") % 4 == 0)
            & ((year("date") % 100 != 0) | (year("date") % 400 == 0)),
            1,
        ).otherwise(0),
    )
    ...
    .withColumn(
        "is_weekend",
        when((dayofweek("date") == 1) | (dayofweek("date") == 7), 1).otherwise(0),
    )
)

df.write.format("delta").mode("overwrite").saveAsTable(
    "health_dw.silver.dim_date_staging"
)
