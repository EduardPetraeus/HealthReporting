...
CREATE OR REPLACE TABLE silver.resting_heart_rate__staging AS
WITH deduped AS (
    SELECT
        *,
        ROW_NUMBER() OVER (
    ...
    FROM bronze.stg_apple_health_resting_heart_rate
...
-- Step 2: Merge staging into silver.resting_heart_rate
MERGE INTO silver.resting_heart_rate AS target
USING silver.resting_heart_rate__staging AS src
...
DROP TABLE IF EXISTS silver.resting_heart_rate__staging;
