-- merge_apple_health_physical_effort.sql
-- Per-source merge: Apple Health -> silver.physical_effort
-- Estimated physical effort/intensity during activity
--
-- Usage: python run_merge.py silver/merge_apple_health_physical_effort.sql

-- Step 1: Create staging table with deduplication
CREATE OR REPLACE TABLE silver.physical_effort__staging AS
WITH deduped AS (
    SELECT
        *,
        ROW_NUMBER() OVER (
    ...
    FROM bronze.stg_apple_health_physical_effort
...
-- Step 2: Merge staging into silver.physical_effort
MERGE INTO silver.physical_effort AS target
USING silver.physical_effort__staging AS src
...
DROP TABLE IF EXISTS silver.physical_effort__staging;
