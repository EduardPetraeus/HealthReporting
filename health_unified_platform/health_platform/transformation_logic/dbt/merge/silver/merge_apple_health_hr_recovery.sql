-- merge_apple_health_hr_recovery.sql
-- Per-source merge: Apple Health -> silver.hr_recovery
-- Heart rate recovery one minute after exercise
--
-- Usage: python run_merge.py silver/merge_apple_health_hr_recovery.sql

-- Step 1: Create staging table with deduplication
CREATE OR REPLACE TABLE silver.hr_recovery__staging AS
WITH deduped AS (
    SELECT
        *,
        ROW_NUMBER() OVER (
    ...
    FROM bronze.stg_apple_health_hr_recovery
...
-- Step 2: Merge staging into silver.hr_recovery
MERGE INTO silver.hr_recovery AS target
USING silver.hr_recovery__staging AS src
...
DROP TABLE IF EXISTS silver.hr_recovery__staging;
