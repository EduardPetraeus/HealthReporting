-- merge_apple_health_flights_climbed.sql
-- Per-source merge: Apple Health -> silver.flights_climbed
-- Flights of stairs climbed per session
--
-- Usage: python run_merge.py silver/merge_apple_health_flights_climbed.sql

-- Step 1: Create staging table with deduplication
CREATE OR REPLACE TABLE silver.flights_climbed__staging AS
WITH deduped AS (
    SELECT
        *,
        ROW_NUMBER() OVER (
    ...
    FROM bronze.stg_apple_health_flights_climbed
...
-- Step 2: Merge staging into silver.flights_climbed
MERGE INTO silver.flights_climbed AS target
USING silver.flights_climbed__staging AS src
...
DROP TABLE IF EXISTS silver.flights_climbed__staging;
