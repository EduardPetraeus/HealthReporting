-- merge_apple_health_hrv.sql
-- Per-source merge: Apple Health -> silver.hrv
-- Heart rate variability (SDNN) readings
--
-- Usage: python run_merge.py silver/merge_apple_health_hrv.sql

-- Step 1: Create staging table with deduplication
CREATE OR REPLACE TABLE silver.hrv__staging AS
WITH deduped AS (
    SELECT
        *,
        ROW_NUMBER() OVER (
    ...
    FROM bronze.stg_apple_health_hrv
...
-- Step 2: Merge staging into silver.hrv
MERGE INTO silver.hrv AS target
USING silver.hrv__staging AS src
...
DROP TABLE IF EXISTS silver.hrv__staging;
