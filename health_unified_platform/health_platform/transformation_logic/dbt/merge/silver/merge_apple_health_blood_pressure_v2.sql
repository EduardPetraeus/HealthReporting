...
-- Note: existing silver.blood_pressure (Withings-only) stays until A7 deprecation
--
-- Sources:
...
WITH sys_deduped AS (
    SELECT *, ROW_NUMBER() OVER (
        PARTITION BY startDate ORDER BY _ingested_at DESC
    ) AS rn
    FROM bronze.stg_apple_health_bp_systolic
...
dia_deduped AS (
    ...
    FROM bronze.stg_apple_health_bp_diastolic
    WHERE startDate IS NOT NULL
),
combined AS (
    SELECT
        s.startDate,
        s.endDate,
        s.value::DOUBLE AS systolic_mmhg,
        d.value::DOUBLE AS diastolic_mmhg,
        s.sourceName
    FROM sys_deduped s
    INNER JOIN dia_deduped d ON s.startDate = d.startDate
...
SELECT
    ...
    md5(
        coalesce(cast(startDate AS VARCHAR), '') || '||' || 'apple_health'
    ) AS business_key_hash,
    md5(
        cast(startDate AS VARCHAR) || '||' ||
        cast(systolic_mmhg AS VARCHAR) || '||' ||
        cast(diastolic_mmhg AS VARCHAR)
    ) AS row_hash,
...
WHEN NOT MATCHED THEN
  INSERT (
    sk_date, sk_time, timestamp, systolic_mmhg, diastolic_mmhg, source_name,
    business_key_hash, row_hash, load_datetime, update_datetime
  )
  VALUES (
    src.sk_date, src.sk_time, src.timestamp, src.systolic_mmhg, src.diastolic_mmhg, src.source_name,
