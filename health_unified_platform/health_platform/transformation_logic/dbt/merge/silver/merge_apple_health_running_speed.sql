-- merge_apple_health_running_speed.sql
-- Per-source merge: Apple Health -> silver.running_speed
-- Running speed in meters per second per session
--
-- Usage: python run_merge.py silver/merge_apple_health_running_speed.sql

-- Step 1: Create staging table with deduplication
CREATE OR REPLACE TABLE silver.running_speed__staging AS
WITH deduped AS (
    SELECT
        *,
        ROW_NUMBER() OVER (
    ...
    FROM bronze.stg_apple_health_running_speed
...
-- Step 2: Merge staging into silver.running_speed
MERGE INTO silver.running_speed AS target
USING silver.running_speed__staging AS src
...
DROP TABLE IF EXISTS silver.running_speed__staging;
