-- merge_apple_health_hand_washing.sql
-- Per-source merge: Apple Health -> silver.hand_washing
-- Hand washing event with duration
--
-- Usage: python run_merge.py silver/merge_apple_health_hand_washing.sql

-- Step 1: Create staging table with deduplication
CREATE OR REPLACE TABLE silver.hand_washing__staging AS
WITH deduped AS (
    SELECT
        *,
        ROW_NUMBER() OVER (
    ...
    FROM bronze.stg_apple_health_hand_washing
...
-- Step 2: Merge staging into silver.hand_washing
MERGE INTO silver.hand_washing AS target
USING silver.hand_washing__staging AS src
...
DROP TABLE IF EXISTS silver.hand_washing__staging;
